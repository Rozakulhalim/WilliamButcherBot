...
from wbb import app2, BOT_USERNAME
...
usage: `ytsmx7 [Movie name] [-limit] [-quality]`
Example: `ytsmx7 lion king -l10 -q1080p`
"""


@app.on_message(filters.command("ytsmx7"))
@capture_err
async def ytsmx(client, message):
    qual = None
    max_limit = 5
    ...
    if get_limit.search(input_) is None and get_quality.search(input_) is None:
        ...
        max_limit = int(get_limit.search(input_).group().strip('-l'))
    if len(input_) == 0:
        await message.reply_text("**No Input**")
    ...
    datas = resp.json()
    if datas['status'] != "ok":
        await message.reply_text("Wrong Status")
        await asyncio.sleep(3)
        await message.delete()
        return
    if datas['data']['movie_count'] == 0 or len(datas['data']) == 3:
        await message.reply_text(f"{_movie} Not Found!")
        await asyncio.sleep(3)
        await message.delete()
        return
    _matches = datas['data']['movie_count']
    await message.reply_text(f"{_matches} Matches Found!, Sending {len(datas['data']['movies'])}.")
